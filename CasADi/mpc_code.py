...
"""
object_methods = [method_name for method_name in dir(ca)
                  if callable(getattr(ca, method_name))]
...
RHS = ca.mtimes(ca.mtimes(rot_3d_z, J), controls)
# maps controls from [va, vb, vc, vd].T to [vx, vy, omega].T
#print(type(states), type(controls), type(RHS))
#print(type(f))
...
for k in range(N):
    ...
    con = U[:, k]
    cost_fn = cost_fn \
        + ca.mtimes(ca.mtimes((st - P[n_states:]).T, Q), (st - P[n_states:])) \
        + ca.mtimes(ca.mtimes(con.T, R), con)
    st_next = X[:, k+1]
    #print(st, con)
    st_next_RK4 = st + (step_horizon / 6) * 1
...
if __name__ == '__main__':
    ...
    print('\n\n')
    print('Number of iterations: ', mpc_iter)
