...
class rov_phone_Pub(object):
        def __init__(self):
                ...
                HOST = sys.argv[1]
                PORT = 1233
                self.rs = socket.socket()  # instantiate
                while True:
                        try:
                                self.rs.connect((HOST, PORT))  # connect to the server
                                break
                        except:
                                try:
                                        #s.shutdown(socket.SHUT_RDWR)
                ...
                self.Firstmag = 0
                self.acceptrange = 1.1 #1.1 - 1.0 = 0.1 = 10%
                self.Linacc = 1
        ...
        def read_and_pub(self):
                ...
                message = "QO" + str(orient) + "EW" + str(gyroeul) + "LA" + str(accels) + "end"
                self.rs.send(message.encode())
